...
> 계층) + SPEC-ANALYZER-TRAIN-TUNING-001(매월 1일 06:00 KST `monthly-optuna-tuning` cron으로
> 캠페인 CLI를 원격 재사용 디스패치해 Optuna 하이퍼파라미터를 재튜닝하고, 조합별 보존 정책 +
> 운영자 수동 롤백 CLI로 안전망을 갖춘 3번째 cron 잡) + SPEC-ANALYZER-INFER-001(매니페스트 기반
> 모델 해석 + 등급 경계 실측 + 피처 조립·예측·score/신뢰도 산출 + `trading_signals`/
> `signal_price_bands` INSERT + `stream:signal:{market}` 발행 + Prometheus 계측의 9개
> 완성 순수 모듈) + SPEC-ANALYZER-PIPELINE-001(그 9개 모듈을 실제 종단간 흐름으로 조립하는
> `inference/pipeline.py` 오케스트레이션 계층 신설 — `python -m analyzer.inference --market
> <market>`가 실제로 추론을 수행하며, 자식 프로세스 종료 후에도 메트릭이 부모의 `/metrics`에서
> 관측되도록 Prometheus 공식 멀티프로세스 모드를 도입) 범위로 한정된다.
...
├── training/        # 모델 학습 코어 — 데이터셋 조립, walk-forward 검증, LightGBM+XGBoost 학습, Optuna 튜닝, 앙상블 스코어링, SHA-256 무결성 검증 모델 영속화(진입점: python -m analyzer.training.train) + 역사적 Walk-Forward 캠페인 평가/안정화 게이트/챔피언 선정(진입점: python -m analyzer.training.campaign, --n-trials 선택 인자, monthly-optuna-tuning cron 등록) + 운영자 수동 롤백 CLI(진입점: python -m analyzer.training.rollback --algorithm {lightgbm,xgboost})
├── inference/       # 자식 CLI 추론 진입점 — 매니페스트 해석·피처 조립·예측·등급 분류·INSERT·발행을 실제로 종단간 실행하는 오케스트레이션(진입점: python -m analyzer.inference --market {domestic,overseas})
